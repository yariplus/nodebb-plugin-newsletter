...
// Import base modules.
const { db, Emailer, User, Meta, SocketAdmin, async, winston, nconf, path } = require('./nodebb.js')
...
exports.filterEmailModify = (data, next) => {
  if ((data.template === 'newsletter' || data.template === 'newsletter_plaintext') && !__prefixTitle) {
    data.subject = data.subject.replace(`[${Meta.config.title}] `, '')
  }
